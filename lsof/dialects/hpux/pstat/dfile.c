...
ncache_alloc()
{
	...
	{
	    (void) fprintf(stderr,
		"%s: can't allocate %ld local name cache entries\n", Pn, Nceh);
	    Exit(1);
	}
	if (Ncfsid)
	    return;
	...
	{
	    (void) fprintf(stderr,
		"%s: can't allocate %ld local file system cache entries\n",
		Pn, NFSIDH);
	    Exit(1);
	}
}
	...
	*fh = f;
	while ((nr = pstat_getmpathname(&mp[0], NPSTM, DNLCINCR, x, fsid)) > 0)
	{
		...
		if (!(nn = (struct l_nc *)malloc(sizeof(struct l_nc)))) {
			...
			Pn, sizeof(struct l_nc), mp[i].psr_name);
		    Exit(1);
		}
	    /*
 ...
 * Save node number.
 */
	Lf->inode = (unsigned long)pd->psfd_ino;
	Lf->inp_ty = 1;
/*
 * Save link count.
 */
