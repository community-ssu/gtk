...
main (int argc, char **argv)
{
	...
	if (1) {
		...
		handle1 = add_file_monitor (FILE1);
		handle2 = add_file_monitor (FILE2);
		handle3 = add_file_monitor (FILE3);
		handle4 = add_file_monitor (FILE4);
		...
		g_print ("Create file '%s'... ", FILE4_LOWER);
		run_and_block_file (test_create_file, FILE4_LOWER, TRUE);
		g_print ("OK\n");

		g_print ("Remove file '%s'... ", FILE4_LOWER);
		run_and_block_file (test_remove_file, FILE4_LOWER, TRUE);
		g_print ("OK\n");
		
		cancel_monitor (handle1);
		cancel_monitor (handle2);
		cancel_monitor (handle3);
		cancel_monitor (handle4);
