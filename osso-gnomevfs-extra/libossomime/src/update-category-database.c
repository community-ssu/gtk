/* This is cut-and-pasted from shared-mime-info and modified. */
#include <config.h>
...
#define OSSO_NS "http://nokia.com/osso/mime-categories"
...
/* Maps categories to types */
static GHashTable *categories_hash = NULL;
...
static gboolean category_is_valid (const char *category)
{
	int i;
...
static void load_type(Type *type, xmlNode *node, GError **error)
{
	...
	for (field = node->xmlChildrenNode; field; field = field->next)
	{
		...
		else if (field->ns && strcmp(field->ns->href, OSSO_NS) == 0)
		{
			char *category_name;
			
			category_name = xmlGetNsProp (field, "name", NULL);

			if (!category_name) 
			{
				g_set_error(error, MIME_ERROR, 0,
					    _("Missing 'name' attribute in <osso:category> "
					      "element"));
				
			}
			else if (!category_is_valid (category_name))
			{
				g_set_error(error, MIME_ERROR, 0,
					    _("Invalid category name '%s' in <osso:category> "
...
/* Write out one line of the 'globs' file */
static void write_out_category(gpointer key, gpointer value, gpointer data)
{
	const gchar *category = (gchar *) key;
	GList *l = value;
	FILE *stream = (FILE *) data;
	...
	for (; l; l = l->next)
	{
		...
		fprintf (stream, " %s/%s", type->media, type->subtype);
	}
...
int main(int argc, char **argv)
{
	...
	namespace_hash = g_hash_table_new_full(g_str_hash, g_str_equal,
					g_free, NULL);
	categories_hash = g_hash_table_new_full(g_str_hash, g_str_equal,
						g_free, NULL);
	...
	{
		FILE *categories;
		char *categories_path;
		categories_path = g_strconcat(mime_dir, "/categories.new", NULL);
		categories = fopen(categories_path, "wb");
		if (!categories)
		...
		g_hash_table_foreach(categories_hash, write_out_category, categories);
		fclose(categories);

		atomic_update(categories_path);
		g_free(categories_path);
	}
#ifndef JUST_WRITE_CATEGORIES
	{
		FILE *globs;
	...
	g_hash_table_destroy(categories_hash);
