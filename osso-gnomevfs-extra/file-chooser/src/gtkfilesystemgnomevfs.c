...
struct _GtkFileSystemGnomeVFS
{
  ...
  guint volume_mounted_id;
  guint volume_unmounted_id;
  guint drive_connected_id;
  guint drive_disconnected_id;
};
...
gtk_file_system_gnome_vfs_init (GtkFileSystemGnomeVFS *system_vfs)
{
  ...
  system_vfs->volume_mounted_id =
    g_signal_connect_object (system_vfs->volume_monitor, "volume_mounted",
			     G_CALLBACK (volume_mount_unmount_cb), system_vfs, 0);
  system_vfs->volume_unmounted_id =
    g_signal_connect_object (system_vfs->volume_monitor, "volume_unmounted",
			     G_CALLBACK (volume_mount_unmount_cb), system_vfs, 0);
  system_vfs->drive_connected_id =
    g_signal_connect_object (system_vfs->volume_monitor, "drive_connected",
			     G_CALLBACK (drive_connect_disconnect_cb), system_vfs, 0);
  system_vfs->drive_disconnected_id =
    g_signal_connect_object (system_vfs->volume_monitor, "drive_disconnected",
			     G_CALLBACK (drive_connect_disconnect_cb), system_vfs, 0);
}

static void
disconnect_if_connected (GnomeVFSVolumeMonitor *monitor, guint signal_id)
{
  if (g_signal_handler_is_connected (monitor, signal_id))
...
gtk_file_system_gnome_vfs_finalize (GObject *object)
{
  ...
  /* Work-around for bug in glib when using g_signal_connect_object to get the
   * closures really freed.
   */
  disconnect_if_connected (system_vfs->volume_monitor, system_vfs->volume_mounted_id);
  disconnect_if_connected (system_vfs->volume_monitor, system_vfs->volume_unmounted_id);
  disconnect_if_connected (system_vfs->volume_monitor, system_vfs->drive_connected_id);
  disconnect_if_connected (system_vfs->volume_monitor, system_vfs->drive_disconnected_id);
					 ...
					 GError        **error)
{
  GtkFileFolderGnomeVFS *folder_vfs = GTK_FILE_FOLDER_GNOME_VFS (folder);
  GnomeVFSFileInfo *info;
  gchar *scheme = gnome_vfs_get_uri_scheme (folder_vfs->uri);
  gboolean retval;
  
  retval = (strcmp (scheme, "file") == 0);
  ...
  if (retval)
    {
      ...
      info = gnome_vfs_file_info_new ();
      result = gnome_vfs_get_file_info (folder_vfs->uri, info, 
      ...
      if (result != GNOME_VFS_OK) 
	{
      ...
      /* If we don't get any file info we assume the file is accessible */
      if (info->valid_fields & GNOME_VFS_FILE_INFO_FIELDS_ACCESS)
	executable = info->permissions & GNOME_VFS_PERM_ACCESS_EXECUTABLE;
      else
	executable = TRUE;
      ...
      if (!executable)
	{
	  g_set_error (error,
		       ...
		       "error accessing '%s': %s",
		       folder_vfs->uri,
		       gnome_vfs_result_to_string (GNOME_VFS_ERROR_ACCESS_DENIED));
	  return FALSE;
	}
    }
	       ...
	       GError       **error)
{
  ...
  switch (result)
    ...
    case GNOME_VFS_ERROR_SERVICE_NOT_AVAILABLE:
      errcode = GTK_FILE_SYSTEM_ERROR_TIMEOUT;
      break;
