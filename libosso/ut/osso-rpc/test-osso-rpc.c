...
int test_osso_rpc_run_with_invalid_osso( void )
{
    ...
    r = osso_rpc_run(NULL,TOP_SERVICE, TOP_OBJECT, TOP_IFACE,
		     ...
		     "print This is a test", DBUS_TYPE_INVALID);
...
int test_osso_rpc_run_multiple_args( void )
{
    ...
    r = osso_rpc_run(osso, TOP_SERVICE, TOP_OBJECT, TOP_IFACE, 
		     LAUNCH_METHOD, NULL, DBUS_TYPE_STRING,
		     "print This is a test", DBUS_TYPE_STRING,
		     "another simple string",DBUS_TYPE_STRING,
		     "one more",DBUS_TYPE_STRING,
		     "and even more",DBUS_TYPE_INVALID);
...
int test_sending_all_types( void )
{
    ...
    assert(foobar.osso != NULL);
    
    foobar.count = 0;
    foobar.ret = OSSO_OK;
    foobar.type = DBUS_TYPE_NIL;
...
gboolean rpc_run_ret_cb2(gpointer data)
{
    ...
    switch(foobar->type) {
      case DBUS_TYPE_NIL:
	dprint("Sending:");
	foobar->send.type = DBUS_TYPE_NIL;
	foobar->send.value.s = NULL;
	_print(&foobar->send);
	...
	r = osso_rpc_run_with_defaults(foobar->osso, TEST_APP_NAME,
                                       "echo",&(foobar->retval),
				       foobar->send.type, DBUS_TYPE_INVALID);
	assert(r == OSSO_OK);
...
gboolean _compare(osso_rpc_t *a, osso_rpc_t *b)
{
    ...
    if((a->type == DBUS_TYPE_NIL) && (b->type == DBUS_TYPE_STRING) &&
       (b->value.s == NULL))
	return TRUE;
    else if((b->type == DBUS_TYPE_NIL) && (a->type == DBUS_TYPE_STRING) &&
       (a->value.s == NULL))
	return TRUE;
    if(a->type != b->type)
	return FALSE;
...
int test_osso_rpc_async_run_and_return( void )
{
    ...
    r = osso_rpc_async_run(osso, TEST_SERVICE, TEST_OBJECT, TEST_IFACE, 
			   ...
			   DBUS_TYPE_INT32, 903, DBUS_TYPE_INVALID);
