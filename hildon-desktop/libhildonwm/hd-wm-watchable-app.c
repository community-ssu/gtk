...
typedef enum 
{
  HDWM_APP_CAN_HIBERNATE  = 1 << 0,
  HDWM_APP_HIBERNATING    = 1 << 1,
  HDWM_APP_MINIMISED      = 1 << 2,
  HDWM_APP_STARTUP_NOTIFY = 1 << 3,
  HDWM_APP_DUMMY          = 1 << 4,
  HDWM_APP_LAUNCHING      = 1 << 5,
  /*
     Last dummy value for compile time check.
     If you need values > HDWM_APP_LAST, you have to change the
     definition of HDWMWatchableAppFlags to get more space.
  */
  HDWM_APP_LAST           = 1 << 7
}HDWMWatchableAppFlagsEnum;
...
typedef char __app_compile_time_check_for_flags_size[
     (guint)(1<<(sizeof(HDWMWatchableAppFlags)*8))>(guint)HDWM_APP_LAST ? 1:-1
                                                    ];

#define HDWM_APP_SET_FLAG(a,f)    ((a)->flags |= (f))
#define HDWM_APP_UNSET_FLAG(a,f)  ((a)->flags &= ~(f))
#define HDWM_APP_IS_SET_FLAG(a,f) ((a)->flags & (f))

/* watchable apps */

struct HDWMWatchableApp
{
  gchar     *icon_name;
  gchar     *extra_icon;
  gchar     *service;
...
static gboolean
hd_wm_watchable_app_has_windows_find_func (gpointer key,
					   gpointer value,
					   gpointer user_data);
...
hd_wm_watchable_app_new (const char * file)
{
  ...
  HDWM_APP_SET_FLAG (app, HDWM_APP_STARTUP_NOTIFY); /* Default */

  if (startup_notify)
    {
      if(!g_ascii_strcasecmp(startup_notify, "false"))
        HDWM_APP_UNSET_FLAG (app, HDWM_APP_STARTUP_NOTIFY);
					   ...
					   gpointer user_data)
{
  ...
  HN_DBG("Checking %p vs %p", 
	 ...
	 (HDWMWatchableApp *)user_data);

  if (hd_wm_watched_window_get_app(win) == (HDWMWatchableApp *)user_data)
    return TRUE;
...
{
  return (g_hash_table_find (hd_wm_get_watched_windows(),
			     hd_wm_watchable_app_has_windows_find_func,
			     (gpointer)app) != NULL);
}
...
hd_wm_watchable_app_has_any_windows (HDWMWatchableApp *app)
{
  ...
  GHashTable * ht = hd_wm_watchable_app_is_hibernating (app) ?
    hd_wm_get_hibernating_windows() :
    hd_wm_get_watched_windows();
...
{
  return HDWM_APP_IS_SET_FLAG(app, HDWM_APP_HIBERNATING);
}
				   ...
				   gboolean          hibernate)
{
  HN_MARK();
  if(hibernate)
    HDWM_APP_SET_FLAG(app, HDWM_APP_HIBERNATING);
  else
    HDWM_APP_UNSET_FLAG(app, HDWM_APP_HIBERNATING);
}
...
hd_wm_watchable_app_is_able_to_hibernate (HDWMWatchableApp *app)
{
  ...
  return HDWM_APP_IS_SET_FLAG(app, HDWM_APP_CAN_HIBERNATE);
}
...
{
  if(hibernate)
    HDWM_APP_SET_FLAG(app, HDWM_APP_CAN_HIBERNATE);
  else
    HDWM_APP_UNSET_FLAG(app, HDWM_APP_CAN_HIBERNATE);
}
...
{
  return HDWM_APP_IS_SET_FLAG(app, HDWM_APP_MINIMISED);
}

gboolean
hd_wm_watchable_app_has_startup_notify (HDWMWatchableApp *app)
{
  return HDWM_APP_IS_SET_FLAG(app, HDWM_APP_STARTUP_NOTIFY);
}
...
{
  if(minimised)
    HDWM_APP_SET_FLAG(app, HDWM_APP_MINIMISED);
  else
    HDWM_APP_UNSET_FLAG(app, HDWM_APP_MINIMISED);
}
...
hd_wm_watchable_app_launch_banner_show (HDWMWatchableApp *app) 
{
  ...
  info->msg = g_strdup_printf(_(hd_wm_watchable_app_is_hibernating(app) ?
...
hd_wm_watchable_app_launch_banner_timeout (gpointer data)
{
  ...
  if (time_left >= current_banner_timeout 
    ...
    {
      /* Close the banner */
      hd_wm_watchable_app_launch_banner_close( NULL, info );
...
void
hd_wm_watchable_app_set_active_window (HDWMWatchableApp *app, HDWMWatchedWindow * win)
{
...
hd_wm_watchable_app_is_dummy(HDWMWatchableApp *app)
{
  ...
  return HDWM_APP_IS_SET_FLAG(app, HDWM_APP_DUMMY);
}

gboolean
hd_wm_watchable_app_is_launching (HDWMWatchableApp *app)
{
  return HDWM_APP_IS_SET_FLAG(app, HDWM_APP_LAUNCHING);
}
...
{
  if(launching)
    HDWM_APP_SET_FLAG(app, HDWM_APP_LAUNCHING);
  else
...
static void
hd_wm_watchable_app_my_window_foreach_func (gpointer key,
                                            gpointer value,
                                            gpointer user_data)
{
  HDWMWatchedWindow *win;  
  ...
  if (hd_wm_watched_window_get_app(win) == wcd->app)
    wcd->count++;
}
...
hd_wm_watchable_app_n_windows (HDWMWatchableApp *app)
{
  ...
  ht = hd_wm_watchable_app_is_hibernating (app) ?
    hd_wm_get_hibernating_windows() :
    hd_wm_get_watched_windows();
  ...
  g_hash_table_foreach(ht, hd_wm_watchable_app_my_window_foreach_func, &wcd);
...
hd_wm_watchable_app_is_active (HDWMWatchableApp *app)
{
  ...
  if(hd_wm_get_active_window() == app->active_window)
    return TRUE;
