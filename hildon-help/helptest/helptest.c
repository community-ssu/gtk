...
#define ossohelp_exists(key) \
        hildon_help_show( NULL, key, HILDON_HELP_SHOW_JUSTASK )
...
gboolean clicked_cb( GtkButton *button, gpointer data )
{
    ...
    if (dialog_help_key) {
        rc= hildon_help_show( osso, dialog_help_key, HILDON_HELP_SHOW_DIALOG );
                           
        fprintf( stderr, "hildon_help_show() returned: %d\n", (int)rc );
        ...
        if ( strcmp(argv[i],"-v")==0 ) {
            ...
            **/
            ...
            if (strcmp(cmd,"html")==0) {
                ...
                rc= hildon_help_show( osso, argv[i+1],0 );
