                                ...
                                guint flags )
{
   ...
   ULOG_DEBUG ( "hildon_help_show: %s\n", help_id );

    if (flags & HILDON_HELP_SHOW_JUSTASK) {
    ...
    if (flags & HILDON_HELP_SHOW_DIALOG) {
...
void on_help( GtkWidget *dialog, gpointer data )
{
    ...
    osso_return_t rc= hildon_help_show( osso, 
                                        topic_id,
...
/*
    ...
    * Now when handling the event of clicking tab1, this should be called:
        hildon_help_dialog_help_enable( sample_dialog, help_id_1,osso );
    * Similarly, clicking tab2 should invoke this call somewhere
        hildon_help_dialog_help_enable( sample_dialog, help_id_2,osso );
