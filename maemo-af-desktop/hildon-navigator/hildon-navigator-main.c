...
static void reload_plugins(Navigator *tasknav, GList *list)
{
    ...
    for (l = tasknav->plugins ; l ; l = l->next)
    {
        ...
        g_object_set (G_OBJECT (plugin->button), 
                      "can-focus", TN_DEFAULT_FOCUS, NULL);
...
#if 0
static gboolean
main_window_focus_out_cb(GtkWidget     *widget,
						 GdkEventFocus *event,
						 gpointer       user_data)
{
	HN_DBG("Main window lost focus");

	/* make us unfocusable again */
...
set_focus_forall_cb (GtkWidget *widget, gpointer data)
{
	...
	HN_DBG("widget of type [%s]", g_type_name(G_TYPE_FROM_INSTANCE(widget)));
	...
	if(GTK_IS_CONTAINER(widget))
	{
		...
		gtk_container_forall (GTK_CONTAINER(widget), set_focus_forall_cb,
...
static void create_navigator(Navigator *tasknav)
{
    ...
    gtk_window_set_accept_focus(GTK_WINDOW(tasknav->main_window),
								TN_DEFAULT_FOCUS);
  ...
  tasknav->app_switcher = hn_app_switcher_new ();
  gtk_box_pack_end (tasknav->box, tasknav->app_switcher, FALSE, FALSE, 0);
  gtk_widget_show (tasknav->app_switcher);
  ...
  /* ensure TN buttons are not focusable to start with*/
  task_navigator_set_focus(FALSE);
...
static void destroy_navigator(Navigator *tasknav)
{
    ...
    gtk_widget_destroy (tasknav->app_switcher);
...
#endif
    tasknav->app_switcher_dnotify_cb = hn_wm_dnotify_cb;

    /* Initialize others menu */
    others_menu_initialize_menu(tasknav->others_menu, hn_wm_dnotify_cb);

}

GtkWidget *
task_navigator_get_button(gint what)
{
	g_return_val_if_fail(what < HN_TN_ACTIVATE_LAST && task_nav, NULL);
	
	if(what == HN_TN_ACTIVATE_OTHERS_MENU)
	{
		return task_nav->others_menu_button;
	}
	else if(what == HN_TN_ACTIVATE_MAIN_MENU)
	{
		return NULL;
	}
	else
	{
		...
		GList *l;
		gint i;
		for (l = task_nav->plugins, i = HN_TN_ACTIVATE_PLUGIN1_MENU;
			 l && i < HN_TN_ACTIVATE_LAST;
			 l = l->next, ++i)
		{
			NavigatorPlugin *plugin = (NavigatorPlugin *)l->data;
			...
			if ((what == HN_TN_ACTIVATE_PLUGIN1_MENU &&
				 !strcmp(LIBTN_BOOKMARK_PLUGIN, plugin->library))
				||
				(what == HN_TN_ACTIVATE_PLUGIN2_MENU &&
				 !strcmp(LIBTN_MAIL_PLUGIN, plugin->library)))
			{
...
void
task_navigator_set_focus(gboolean focus)
{
	g_return_if_fail(task_nav && task_nav->main_window && task_nav->box);

	if(task_nav->focusable == focus)
	{
		HN_DBG ("requested focus state [%s] already current",
				focus ? "TRUE":"FALSE");
		return;
	}
	
	gtk_window_set_accept_focus(GTK_WINDOW(task_nav->main_window), focus);

#ifndef TN_ALWAYS_FOCUSABLE
	gtk_container_forall (GTK_CONTAINER(task_nav->box), set_focus_forall_cb,
						   GINT_TO_POINTER(focus));
#endif

	task_nav->focusable = focus;
	
	if (focus)
	{
		gtk_window_present(GTK_WINDOW(task_nav->main_window));
	}
	else
	{
		gtk_window_set_focus (GTK_WINDOW(task_nav->main_window), NULL);
	}
}
