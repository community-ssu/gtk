...
hn_wm_top_view (GtkMenuItem *menuitem)
{
  ...
  if (win && hn_wm_watched_window_get_app (win))
      ...
      if (hn_wm_watched_window_is_hibernating(win))
	{
	  ...
	  hn_wm_top_service(hn_wm_watchable_app_get_exec (app));
	  return;
	}
						   ...
						   gpointer user_data)
{
  ...
  if (!strcmp(hn_wm_watchable_app_get_exec (app), 
	      (gchar*)user_data))
    return TRUE;
...
hn_wm_lookup_watched_window_via_service (const gchar *service_name)
{
  ...
  /* NOTE: Serivce references the app_get_exec field.
   *       which actually references 'X-Osso-Service' .desktop key
   *
   * FIXME: Should rename 'exec' to service in watchable_app API. 
  */
						  ...
						  gpointer user_data)
{
  ...
  if (hn_wm_watchable_app_get_exec (app) == NULL)
    return FALSE;

  if (!strcmp(hn_wm_watchable_app_get_exec (app), (gchar*)user_data))
    return TRUE;
...
static DBusHandlerResult 
hn_wm_dbus_exit_handler (DBusConnection *connection,
			 DBusMessage    *message,
			 void           *data )
{
...
hn_wm_init (ApplicationSwitcher_t *as)
{
  ...
  if (!connection ) 
  ...
  else 
    {
      ...
      dbus_connection_add_filter( connection, hn_wm_dbus_exit_handler,
				  /* model */ NULL, NULL);
