...
static gint home_bg_image_select_mode = CENTERED;
...
static void hildon_home_load_configure(void);
    ...
    switch (gtk_dialog_run(GTK_DIALOG(dialog)))
    {
      case GTK_RESPONSE_OK:	
        ...
        if(home_bg_image_previewed == FALSE 
           ...
           || home_bg_image_select_mode != home_bg_scale_combobox_active_item)
        {
...
void show_file_corrupt_note(void)
{
    ...
    file_corrupt_note =
      hildon_note_new_information(GTK_WINDOW(window), HILDON_HOME_CORRUPTED_TEXT);
...
{    
    ...
    gint full_exit_status = child_exit_status;
    ...
    case HILDON_HOME_IMAGE_LOADER_OK:
        ...
        if(full_exit_status == 0) /* Operation was not cancelled either */
        {
            home_bg_image_select_mode = home_bg_scale_combobox_active_item;
 ...
 * @hildon_home_save_configure
 * 
 * saves bg image selection values to configure file
 */
static 
 ...
 * @hildon_home_load_configure
 * 
 * load bg image selection values from configure file
 */
static 
void hildon_home_load_configure()
{
    FILE *fp;
    gchar *configure_file;
    ...
    home_bg_scale_combobox_active_item = home_bg_image_select_mode;
