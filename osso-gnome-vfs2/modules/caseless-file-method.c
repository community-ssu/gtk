...
/* file-method.c - Local file access method for the GNOME Virtual File
   ...
   Case insensitivity added in Sept, 2004-2006
	 ...
	 GnomeVFSContext *context)
{
	...
	*method_handle = (GnomeVFSMethodHandle *) file_handle;
	...
	caseless_file_method_clear_cache ();
	 ...
	 GnomeVFSContext *context)
{
	...
	if (caseless_file_method_is_file_open (new_full_name) || 
	    caseless_file_method_is_file_open (old_full_name)) {
		g_free (old_full_name);
		g_free (new_full_name);
	   ...
	   GnomeVFSContext *context)
{
	...
	if (caseless_file_method_is_file_open (full_name)) {
		g_free (full_name);
		return GNOME_VFS_ERROR_LOCKED;
	}
		...
		GnomeVFSMonitorType monitor_type)
{
	 ...
	 *
	 * MJR: Removed the do_is_local() check here, since it 
     * always seems to fail for local files?
 	 *
	 */
